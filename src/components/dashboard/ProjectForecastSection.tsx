...
export const ProjectForecastSection: React.FC<ProjectForecastSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-indigo-700 dark:text-indigo-300 font-bold bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/20">
        ...
        <div className={`p-4 rounded-2xl space-y-1 shadow-sm ${getEVMCardClass(portfolioForecasts.avgPortfolioSPI, 0.9)}`}>
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
            ...
            {portfolioForecasts.avgPortfolioSPI < 0.9 ? (
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Target: 1.00</span>
            )}
          </div>
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1 shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Delayed / At Risk</span>
          </div>
          <p className="text-[11px] text-slate-500 truncate">
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Worst-case</span>
          </div>
          <p className={`text-xl sm:text-2xl font-black font-mono ${portfolioForecasts.maxSlippageDays > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-5 rounded-2xl space-y-4 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="flex items-center gap-2 flex-wrap w-full md:w-auto">
            {/* Chart Mode Switcher */}
            <div className="flex items-center bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800 flex-wrap gap-1">
              <button
            ...
            {activeChartMode !== 'comparison' && (
              <select
                ...
                className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-800 dark:text-white rounded-xl px-3 py-1.5 focus:outline-none focus:border-indigo-500 shadow-sm"
              >
                {projects.map(p => (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 rounded-xl px-2.5 py-1.5 focus:outline-none focus:border-indigo-500 shadow-sm"
            >
          ...
          {filteredForecasts.map(projForecast => {
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-slate-900 border rounded-2xl p-4 sm:p-5 transition-all space-y-4 shadow-sm flex flex-col justify-between ${
                  isSelected
                    ? 'border-indigo-500 ring-1 ring-indigo-500/50 bg-indigo-50/20 dark:bg-slate-900/90'
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30">
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/60 text-center text-xs">
                  <div className={`rounded-lg p-1 transition-all ${projForecast.currentSPI < 0.9 ? 'bg-rose-50 dark:bg-rose-500/15 border border-rose-200 dark:border-rose-500/60 shadow-[0_0_8px_rgba(244,63,94,0.25)]' : ''}`}>
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">
                      SPI {projForecast.currentSPI < 0.9 && <span className="text-rose-600 dark:text-rose-400 font-bold text-[9px]">(&lt;0.9)</span>}
                    </span>
                    <span className={`font-bold font-mono ${getEVMTextColorClass(projForecast.currentSPI, 0.9)}`}>
                      {projForecast.currentSPI.toFixed(2)}
                    </span>
                  </div>
                  <div className={`rounded-lg p-1 transition-all ${projForecast.currentCPI < 0.9 ? 'bg-rose-50 dark:bg-rose-500/15 border border-rose-200 dark:border-rose-500/60 shadow-[0_0_8px_rgba(244,63,94,0.25)]' : ''}`}>
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">
                      CPI {projForecast.currentCPI < 0.9 && <span className="text-rose-600 dark:text-rose-400 font-bold text-[9px]">(&lt;0.9)</span>}
                    </span>
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-semibold transition-colors"
                  >
